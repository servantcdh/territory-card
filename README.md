...
1. 레트로 컨셉으로 직접 디자인을 하는 중인데 역시 `Nestjs`로 뒷단을 만들 때랑은 다른 맛이 있다. 물론 잘 만들어진 컴포넌트 라이브러리들을 가져다 쓸 수 있겠지만 이 프로젝트의 목적 중 하나가 `webpack`, `React`, `tailwindcss` 숙련도 향상임을 고려해 왠만해선 직접 구현을 하기로 마음을 먹었다. 확실히 도움이 된다. 앞으로 이 곳이 내 개발 보물 창고가 될 예정...

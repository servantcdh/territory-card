...
1. 아토믹 디자인을 적용한 폴더 구조를 생성하면서 폴더들이 알파벳 순서로 이쁘게 정리되는게 참 마음에 들었다  
   이제부터 하나씩 만들어가면서 느끼겠지만 분명히 아토믹 패턴으로는 나눌 수 없는 컴포넌트들이 있을 것이다. 뭐 장단이 있겠지만 몇 가지 아톰들을 만들면서 재미를 느꼈다.  
   체계적인 구조에 인상받아서 그런건지, 직접 디자인을 해서 그런지 아니면 결과물을 사용할 사람들의 반응이 기대되기 때문인지 모르겠지만 아무튼 즐겁게 작업할 수 있을 듯하다.
